...
/*
-----------------------------------------------------------------------------------------------------------------------
   desynpuf_id conversion function from bytea to bigint which can be used on any given table
-----------------------------------------------------------------------------------------------------------------------
*/
...
BEGIN
	...
	EXECUTE format('UPDATE %I SET %I = RIGHT(%I::TEXT, -1)::BIT(64)::BIGINT', table_name, 'desynpuf_id', 'col_old');
	EXECUTE format('ALTER TABLE %I DROP COLUMN %I', table_name, 'col_old');

	-- Return the first 5 entries of the reformatted desynpuf_id as proof the function worked as expected

	s := format('SELECT %I FROM %I LIMIT 5;', 'desynpuf_id', table_name);
...
-- drop all single segments columns that were created for conversion
